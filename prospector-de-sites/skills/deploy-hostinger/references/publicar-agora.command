#!/bin/bash
# Prospector de Sites — publica a fila na Hostinger (Mac).
...
U=$(python3 -c "import json;print(json.load(open('$CFG'))['hostinger'].get('usuario',''))")
P=$(python3 -c "import json;print(json.load(open('$CFG'))['hostinger'].get('senha',''))")
SRV=$(python3 -c "import json;print(json.load(open('$CFG'))['hostinger'].get('servidor',''))")
[ -n "$U" ] && [ -n "$P" ] && [ -n "$SRV" ] || { log "ERRO: preencha a conexao Hostinger no dashboard (Configuracoes), incluindo a senha."; fim 1; }
OK=0; FALHA=0
