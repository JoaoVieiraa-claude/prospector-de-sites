...
1. `/setup` — roda uma vez: assinatura, nichos padrão, dados de FTP da Hostinger (com teste de publicação).
...
- Hospedagem Hostinger com acesso ao hPanel (conta de FTP)

## Onde ficam os dados

Tudo na pasta conectada: `prospector-config.json` (preferências e credenciais — a senha de FTP fica em texto no seu computador), `leads.md` (pipeline) e `sites/[slug]/` (páginas criadas).
