...
1. **Extração do conteúdo real — depende do `modo` do lead (em `leads.md`):**
   - **🔄 Redesign** (tem site): abra o site original no Claude in Chrome (o sandbox costuma bloquear fetch direto). Extraia TODO o conteúdo real: textos, serviços, formação/credenciais, endereço, telefone/WhatsApp, e-mail, redes sociais, horários, paleta e — OBRIGATÓRIO — as URLs reais do logo e das fotos (via JS: `img.currentSrc` de todas as imagens; role até o fim se forem lazy-load). Screenshot do site para referência.
   - **✨ Criar site** (sem site): a fonte é o **Instagram** do lead (+ perfil do Google Maps). Abra o `@perfil` no Claude in Chrome e colete: bio, descrição do negócio, serviços/cardápio, endereço, horários, telefone/WhatsApp, e as **URLs reais das melhores fotos** (posts do feed — `img.currentSrc`; priorize fotos boas de produto/ambiente). Pegue o logo/foto de perfil. Complete com o que o Maps mostra (nota, avaliações em destaque como prova social, fotos do estabelecimento). Se não houver logo, use composição tipográfica com o nome.
2. **Redesign / criação**: aplique a skill `redesign-premium` na íntegra. Regra de ouro: NADA inventado — o conteúdo é do cliente (site ou Instagram). Fotos e logo REAIS DEVEM aparecer na página. No modo Criar site, use as avaliações do Google como prova social e monte a estrutura que o negócio nunca teve (hero + serviços/cardápio + fotos + prova social + CTA de WhatsApp).
